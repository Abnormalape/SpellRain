...
{
    public class AttackFirst : Spell
    {
        public AttackFirst(int comboNeed, SpellManager spellholder) : base(comboNeed, spellholder)
        {
        ...
        public override void TryUseSpell(int inputCombo, Player spellUser, PhotonView targetSpawner)
        {
            if (JudgeSpell(inputCombo, comboNeeded))
            {
                ...
                targetSpawner.RPC("OrderSpawnerSpawnWord", targetPlayer, false, comboNeeded, 1, true, 0, (float)comboNeeded);
            }
            else
            {
